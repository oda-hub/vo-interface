...
psql -v ON_ERROR_STOP=1 --username "$POSTGRESQL_USER" -d $POSTGRESQL_DB_NAME <<-EOSQL
    -- Create the view
    CREATE VIEW $POSTGRESQL_DB_SCHEMA.obscore AS
    SELECT 
        ...
        string_agg(SUBSTRING(f_m_1.uri FROM 10), ', ') AS access_url,
    ...
    FROM node_field_data
        ...
        LEFT JOIN path_alias ON path_alias.path::text ~~ ('/node/'::text || node_field_data.nid::text)
        LEFT JOIN node__field_fits_file ON node_field_data.nid = node__field_fits_file.entity_id
        LEFT JOIN file_managed f_m_1 ON node__field_fits_file.field_fits_file_target_id = f_m_1.fid
        LEFT JOIN node__field_image_png ON node_field_data.nid = node__field_image_png.entity_id
        LEFT JOIN file_managed f_m_2 ON node__field_image_png.field_image_png_target_id = f_m_2.fid
    WHERE
